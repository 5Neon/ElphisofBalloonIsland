...
public class CameraSpinner : MonoBehaviour
{
    ...
    void Start()
    {
        airballoontarget = GameObject.FindWithTag("Target");
    }
    ...
    void Update()
    {
        ...
        if (GameManager.state == GameManager.Island.Air)
        {
            MaximumFollowAltitude = 60;
        }
        else
        {
            MaximumFollowAltitude = 30;
        }

        // 카메라가 일정 수직 범위 내에서 이동할 수 있도록(플레이어가 떨어지고 하늘에서 내려오는 장면을 보여주기 위함)
        if (target.position.y > MaximumFollowAltitude)
        {
            transform.position = new Vector3(target.position.x, MaximumFollowAltitude, target.position.z);
        }

        if (target.position.y < MinimumFollowAltitude)
        {
            transform.position = new Vector3(target.position.x, MaximumFollowAltitude, target.position.z);
        }
        ...
        // 카메라 회전
        if (!GameManager.JumpMapTriggerOn || !GameManager.isTalking)
        {
